...
public class GenerateUtils
{
	...
	public static List<Table> getList(String dbClass, String table_name, String url, String MysqlUser, String mysqlPassword, String database, String sql)
	{
		...
		try
		{
			...
			while (rs.next())
			{
				...
				table.setTypeName(StringUtil.lowerCase(rs.getString("COLUMN_NAME")));
				table.setColumnName(StringUtil.uncapitalize(toUpper(StringUtil.lowerCase(rs.getString("COLUMN_NAME"))))); // 将所有的字段名改为小写方便命名
				table.setColumnNameUpper(StringUtil.capitalize(table.getColumnName()));
	...
	public static String toUpper(String str)
	{
		...
		{
			if (name == null)
				name = StringUtil.capitalize(string);
			else
				name += StringUtil.capitalize(string);
		}
		return name;
	...
	private static String sqlTypeToJavaType(String sqlType)
	{
		...
		{
			return "float";
		} else if ("decimal".equalsIgnoreCase(sqlType) || "numeric".equalsIgnoreCase(sqlType) || "real".equalsIgnoreCase(sqlType) || "money".equalsIgnoreCase(sqlType) || "smallmoney".equalsIgnoreCase(sqlType) || "double".equalsIgnoreCase(sqlType))
		{
			return "Double";
		} else if ("varchar".equalsIgnoreCase(sqlType) || "char".equalsIgnoreCase(sqlType) || "nvarchar".equalsIgnoreCase(sqlType) || "nchar".equalsIgnoreCase(sqlType) || "text".equalsIgnoreCase(sqlType))
		{
			return "String";
	...
	private static String sqlTypeToJdbcType(String sqlType)
	{
		...
		{
			return "DECIMAL";
		} else if (sqlType.equalsIgnoreCase("decimal") || sqlType.equalsIgnoreCase("numeric") || sqlType.equalsIgnoreCase("real") || sqlType.equalsIgnoreCase("money") || sqlType.equalsIgnoreCase("smallmoney") || sqlType.equalsIgnoreCase("double"))
		{
			return "DECIMAL";
