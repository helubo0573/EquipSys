...
public class ServletUtils
{
	...
	public static void outputFile(HttpServletResponse response, Resource[] rs, boolean gzip) throws IOException
	{
	...
	public static void outputFile(HttpServletResponse response, Resource r, boolean gzip) throws IOException
	{
		OutputStream output = null;
	...
	public static boolean checkAndSetExpiresHeaders(HttpServletRequest request, HttpServletResponse response, long lastModi)
	{
		...
		if (!ServletUtils.checkIfModifiedSince(request, response, lastModi) || !ServletUtils.checkIfNoneMatchEtag(request, response, etag))
		{
			return false;
		}
	...
	{
		response.addHeader(CONTENT_TYPE, JS_TYPE);
		OutputStreamWriter out = new OutputStreamWriter(response.getOutputStream(), CODE_UTF8);
		JsonUtil.write(out, res);
	}
	...
	public static void writeToResponse(HttpServletResponse response, Object object) throws UnsupportedEncodingException, IOException
	{
		response.addHeader(CONTENT_TYPE, JS_TYPE);
		response.setContentType("application/json");
		response.setCharacterEncoding(CODE_UTF8);
